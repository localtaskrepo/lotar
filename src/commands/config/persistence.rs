...
fn optimize_project_config(mut config: ProjectConfig) -> Result<ProjectConfig, ConfigError> {
    ...
    if let Some(require_assignee) = config.require_assignee {
        if require_assignee == global_config.require_assignee {
            config.require_assignee = None;
        }
    }

    if let Some(require_due_date) = config.require_due_date {
        if require_due_date == global_config.require_due_date {
            config.require_due_date = None;
        }
    }

    // Check numeric defaults
    if let Some(max_title) = config.max_title_length {
        if max_title == global_config.max_title_length {
            config.max_title_length = None;
        }
    }

    if let Some(max_desc) = config.max_description_length {
        if max_desc == global_config.max_description_length {
            config.max_description_length = None;
        }
    }

    if let Some(ref id_format) = config.id_format {
        if *id_format == global_config.id_format {
            config.id_format = None;
        }
    }
