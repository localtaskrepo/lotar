...
use crate::commands::config::operations::*;
use crate::commands::config::persistence::create_config_from_template;
use crate::commands::config::utils::*;
use crate::project;
        ...
        Ok(config_manager) => {
            match config_manager.get_project_config(&project_name) {
                Ok(config) => {
                    ...
                    println!("  ID Format: {}", config.id_format);
                    ...
                    println!("  Require Assignee: {}", config.require_assignee);
                    println!("  Require Due Date: {}", config.require_due_date);
                    println!("  Max Title Length: {}", config.max_title_length);
                    println!("  Max Description Length: {}", config.max_description_length);
                    println!();
...
pub fn handle_set_config(args: &[String]) {
    ...
    match field.as_str() {
        ...
        "require_assignee" => set_require_assignee(value, project_name.as_deref()),
        "require_due_date" => set_require_due_date(value, project_name.as_deref()),
        ...
        _ => {
            ...
            eprintln!("                  default_assignee, default_priority, require_assignee, require_due_date,");
            eprintln!("                  server_port, default_project");
            std::process::exit(1);
        }
    }
}
