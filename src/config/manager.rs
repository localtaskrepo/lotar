...
impl ConfigManager {
    ...
    pub fn ensure_default_prefix(&mut self, tasks_dir: &Path) -> Result<String, ConfigError> {
        ...
        let detected_prefix =
            if let Some(detected) = Self::auto_detect_prefix(&tasks_dir.to_path_buf()) {
                detected
            } else {
    ...
    pub fn update_config_field(
        ...
        if let Some(project) = project_prefix {
            ...
            let mut project_config = Self::load_project_config_from_dir(project, tasks_dir)
            ...
            let mut global_config =
                Self::load_global_config(Some(&tasks_dir.to_path_buf())).unwrap_or_default();
    ...
    fn load_project_config_from_dir(
        ...
        let path = tasks_dir.join(project_name).join("config.yml");
        if !path.exists() {
