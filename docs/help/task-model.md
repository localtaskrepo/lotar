...
- **Multiple storage roots:** reads and searches consider the active tasks root plus sibling workspace `.tasks` roots (monorepo layout). Unfiltered listing, project-filtered listing, full-ID lookup, and bare numeric lookup all discover the same nested-root tasks, using the root that actually holds the task for config, sprint memberships, and attachments. A bare number resolves only when exactly one project/root stores it.
...
- **Ambiguity fails closed:** when the same full ID (or the same number) exists in more than one root, lookups return "not found" or an explicit ambiguity error listing both locations — never an arbitrary task. Mutations additionally refuse cross-root targets before any side effect: a task stored in a sibling workspace root must be edited from inside that workspace, and sync canonicalization stays locked to the primary workspace. Listing endpoints enumerate every stored copy (duplicates appear as separate entries) rather than deduplicating an identity they cannot prove.
